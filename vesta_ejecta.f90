!This program calculates the landing locations of the ejecta particles launched from the Rheasilvia, the largest impact basin on Vesta. 
...
!The equation of motion is performed in a rotating reference frame (i.e. the asteroid-fixed frame) described by Scheeres et al. [2002], Asteroid III, Univ. of Arizona Press. 
     ...
     integer :: loop,azimuth,Target
     real(8) :: pp(3),pv(3),dx(3),dp(3),dx_prev(3),dp_prev(3),dt,g,launch_point
     real(8) :: gm,distance,w, radius,launch,rotation,c_radius
   ...
   pi = acos(-1.0d0)
   angle = 25.                           !initial launch angle of ejecta particle from horizontal
   azimuth = 180.                        !initial launch azimuth (from 0 to 360 degree)  
   dt = 1.d0                             !time step
   radius = 525.4d0*0.5d0*1000.d0        !radius of the asteroid (Vesta) [m]
   mass_asteroid   =2.59d20              !mass of the asteroid (Vesta) [kg]
   mass_sun = 1.989d30                   !mass of the Sun [kg]
   semi_major_axis = 2.36d0*1.4959787d8*1000.d0  !s.m.a. of the asteroid [m]
   rotation = 5.342                      !rotation period of the asteroid [h]
   c_radius = 0.5d0*dble(500.*1000.)     ! crater radius (Rheasilvia) [m]
   c_lat = dble(-75.d0) *pi/180.d0       !latitude of the crater center
   c_lon = dble(301) *pi/180.d0          !east longitude of the crater center
