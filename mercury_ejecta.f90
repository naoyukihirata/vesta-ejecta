!This program calculates the landing locations of the ejecta particles launched from the Caloris, the largest impact basin on Mercury. 
...
!The equation of motion is performed in a rotating reference frame (i.e. the asteroid-fixed frame) described by Scheeres et al. [2002], Asteroid III, Univ. of Arizona Press. 
   ...
   pi = acos(-1.0d0)
   angle = 45.                           !initial launch angle of ejecta particle from horizontal
   azimuth = 180.                        !initial launch azimuth (from 0 to 360 degree)  
   dt = 1.d0                             !time step
   radius = 4880.*0.5d0*1000.d0        !radius of the asteroid (mercury) [m]
   mass_asteroid   =3.285d23             !mass of the asteroid (mercury) [kg]
   mass_sun = 1.989d30                   !mass of the center object (Sun) [kg]
   semi_major_axis = 0.387*1.4959787d8*1000.d0  !s.m.a. of the asteroid [m]
   rotation = 176.*24.                   !rotation period of the asteroid [h]
   c_radius = 0.5d0*dble(1550.*1000.)     ! crater radius (Caloris) [m]
   c_lat = dble(30.5) *pi/180.d0       !latitude of the crater center
   c_lon = dble(170.2) *pi/180.d0          !east longitude of the crater center
